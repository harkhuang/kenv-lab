...
# Redis 6节点集群管理脚本
# 用法: ./redis-cluster-6nodes-manager.sh [start|stop|status|create|reset]

set -e

# 配置参数
REDIS_HOME="/usr/local/bin"
CLUSTER_DIR="/home/k/kenv-lab/redis-cluster-6nodes"
NODES=(7011 7012 7013 7014 7015 7016)
HOST="127.0.0.1"
...
create_cluster() {
    ...
    if [ $? -eq 0 ]; then
        log_success "Redis 6节点集群创建成功"
        show_cluster_info
    else
        log_error "Redis 6节点集群创建失败"
        return 1
    fi
}
...
show_cluster_status() {
    ...
    # 检查集群状态
    if redis-cli -p 7011 ping &>/dev/null; then
        echo
        log_info "集群信息:"
        redis-cli -p 7011 cluster info | grep -E "cluster_state|cluster_slots_assigned|cluster_known_nodes"
...
show_cluster_info() {
    echo
    log_info "=== Redis 6节点集群信息 ==="
    redis-cli -p 7011 cluster info
...
main() {
    case "$1" in
        ...
        "reset")
            ...
            ;;
        *)
            echo "用法: $0 {start|create|stop|status|info|reset}"
            echo
            echo "命令说明:"
            echo "  start   - 启动6节点集群节点（不创建集群）"
            echo "  create  - 完整创建并启动6节点集群"
            echo "  stop    - 停止6节点集群"
            echo "  status  - 显示6节点集群状态"
            echo "  info    - 显示详细6节点集群信息"
            echo "  reset   - 重置6节点集群（删除所有数据）"
            exit 1
            ;;
    esac
}
